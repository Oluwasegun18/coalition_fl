...
def safe_acc_loss(M_prime, D_S, L=1, eta=0.01, phi=0.1, T_local=10, beta=0.1, G=1.0, epsilon=1.4):
    den = (1 - (beta * eta / 2) - (L * G * D_S) / (epsilon ** 2))
    # print("Denominator =  ",den)
    if M_prime <= 0 or T_local <= 1 or den <= 0:
        print("D_S", D_S, "den", den)
        return float('inf')
    
    return 1.0 / (M_prime * eta * phi * (T_local - 1) * den)


gammaB_values = np.arange(1500, 10000, 200) #(2400, 10000, 200) # np.arange(1400, 5000, 100) # np.arange(1000, 30000, 500)
...
for cluster_id in range(num_clusters):
    ...
    all_combinations = []
    best_combinations =[]
    stop_gammaB_loop = False
    grand_coalition_cache = None
    prev_partition_signature = None
    temp_coalition = 0
    for gammaB in gammaB_values:
        ...
        if grand_coalition_cache is not None:
            partition_updated = copy.deepcopy(grand_coalition_cache["partition"])
            total_payment = grand_coalition_cache["total_payment"]
            D_S = grand_coalition_cache["D_S"]
            r = grand_coalition_cache["r"]
            ...
            phi, epsilon, G, eta, beta, T_local = 0.1, 1.4, 1.0, 0.01, 0.1, 10
            ...
            for r in np.arange(r_start, r_end + 1, 100):
                ...
                def calc_utility(partition):
                    ...
                    for coalition in partition:
                        ...
                        coalition_rate= len(coalition)-1
                        coalition_r= (15*coalition_rate+20)  #(28*coalition_rate+25)  #+(((r-r_start)/100)*300)

                        for d in coalition:
                            d_k = device_emd[cluster_id][d]
                            c_k = 1 - d_k / 2
                            score_sum = sum(1 - device_emd[cluster_id][di] / 2 for di in coalition)
                            R_k = (c_k / score_sum) * new_c_p * coalition_r
                ...
                max_size = max(len(c) for c in partition_updated)
                partition_signature = tuple(sorted(tuple(sorted(c)) for c in partition_updated))

                print("cluster", cluster_id, "gammaB", gammaB, "r", r, "max coalition", max_size)

                if max_size >= devices_per_cluster or partition_signature == prev_partition_signature:
                    grand_coalition_cache = {
                        "partition": copy.deepcopy(partition_updated),
                ...
                prev_partition_signature = partition_signature
...
# 3️⃣ T values
T_vals = np.arange(12000, 60000, 500) #(2400, 15000, 200) #np.arange(500, 5000, 200)  #   np.arange(5000, 40000, 500)
avg_losses = []
utilities = []
...
for T in T_vals:
    ...
    try:
        ...
        utility = avg_loss + 0.0002 * T # 0.00001   # for r 0.0000208359  0.0000208359
...
for k in range(num_clusters):
    ...
    combinations = all_best_combinations_list[k] #all_combinations_list[k]
...
df.to_csv(f"{path}/optimal_info_CSLRA.csv", index=False)
...
for T_val in T_vals:
    ...
    # Save plotted data to CSV
df = pd.DataFrame(save_optimal_info)
df.to_csv(f"{path}/tval_info_CSLRA.csv", index=False)
