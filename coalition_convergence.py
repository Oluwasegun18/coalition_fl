    ...
    for cluster_id in range(1):
        ...
        for gammaB in gammaB_values:
            ...
            for r in np.arange(r_start, r_end + 1, 20):
                ...
                acc_loss_bound = safe_acc_loss(M_prime, D_S, L, eta, phi, T_local, beta, G, epsilon)
            ...
            rewards = [row[0] for row in EMDs]
            D_S_values = [row[1] for row in EMDs]
            plt.figure(figsize=(10, 6),dpi=600)
            plt.plot(rewards, D_S_values, marker='o', color='black', label=r"$GEMD$ vs Reward")
            ...
            # plt.tight_layout()
            plt.savefig(os.path.join(path, f"EMD_reward_{cluster_id}_gammaB_{gammaB}.pdf"), dpi=600,bbox_inches='tight')
            plt.close()
...
# #     "Utility": utilities[best_idx]
# # })
# # df.to_csv(f"results/optimal_utility.csv", index=False)
...
# #     "loss": avg_losses[best_idx]
# # })
# # df.to_csv(f"results/loss.csv", index=False)
