...
path='results/Benchmark2/result3'
...
gammaB_values = np.arange(1000, 30000, 100) # np.arange(500, 3000, 200) # np.arange(1000, 30000, 500)
        ...
        for r in np.arange(r_start, r_end + 1, 100):
            ...
            def calc_utility(partition):
                ...
                for coalition in partition:
                    ...
                    coalition_r= (20*coalition_rate+20)  #+(((r-r_start)/100)*300)
...
# 3️⃣ T values
T_vals = np.arange(5000, 40000, 500) #np.arange(500, 5000, 200)  #   np.arange(5000, 40000, 500)
avg_losses = []
utilities = []
...
for T in T_vals:
    ...
    try:
        ...
        utility = avg_loss + 0.00001 * T # 0.00001   # for r 0.0000208359  0.0000208359
...
# Save plotted data to CSV
csv_file = "results/optimized_result.csv"
new_row = {
    "option": "Benchmark2",
    "T": T_opt,
...
df.to_csv(f"{path}/optimal_info_Coalation_Based_R.csv", index=False)




T_vals = np.arange(5000, 15500, 500)
save_optimal_info = []  # make it a list, not dict
for T_val in T_vals:
    n_list = [sum(len(cluster_device_labels[k][d]) for d in range(devices_per_cluster)) for k in range(num_clusters)]
    x0 = []
    ...
    # Save plotted data to CSV
df = pd.DataFrame(save_optimal_info)
df.to_csv(f"{path}/tval_info_Coalation_Based_R.csv", index=False)
