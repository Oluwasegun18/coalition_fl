...
print(f"Running benchmark_4_new.py with seed={RUN_SEED}")
...
path = os.environ.get("COALITION_OUTPUT_DIR", "results_new1/cifar10/constant_r/result")
...
gammaB_values = np.arange(1800, 10000, 200) #np.arange(1200, 5000, 100) #np.arange(1000, 30000, 500)
...
df.to_csv(f"{path}/optimal_info_CLRA.csv", index=False)
...
for T_val in T_vals:
    ...
    # Save plotted data to CSV
df = pd.DataFrame(save_optimal_info)
df.to_csv(f"{path}/tval_info_CLRA.csv", index=False)
