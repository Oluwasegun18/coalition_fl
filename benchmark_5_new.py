...
print(f"Running benchmark_5_new.py with seed={RUN_SEED}")
...
path = os.environ.get("COALITION_OUTPUT_DIR", "results_new1/cifar10/minimum_r/result")
...
gammaB_values = np.arange(1000, 10000, 200) #np.arange(100, 3000, 200) #np.arange(1000, 30000, 500)
...
for cluster_id in range(num_clusters):
    ...
    for gammaB in gammaB_values:
        ...
        check_m = []
        for r in np.arange(r_fixed, r_fixed+1, 100):
            ...
            total_payment=sum(rewards.values())
            total_payment1 = sum(device_rewards.values()) #sum(rewards.values())
...
# 3️⃣ T values
T_vals = np.arange(12000, 60000, 500) #np.arange(200, 5000, 200)  #np.arange(5000, 40000, 500)
avg_losses = []
utilities = []
...
for T in T_vals:
    ...
    try:
        ...
        utility = avg_loss + 0.0002 * T  # 0.00001 * T 0.0000213359
...
df.to_csv(f"{path}/optimal_info_MiLRA.csv", index=False)
...
for T_val in T_vals:
    ...
    # Save plotted data to CSV
df = pd.DataFrame(save_optimal_info)
df.to_csv(f"{path}/tval_info_MiLRA.csv", index=False)
