...
path = 'results/Benchmark1/result4'
...
gammaB_values =np.arange(1000, 30000, 100) # np.arange(100, 3000, 200)
...
# 3️⃣ T values
T_vals = np.arange(5000, 40000, 500) #np.arange(200, 5000, 200)
avg_losses = []
utilities = []
...
for T in T_vals:
    ...
    try:
        ...
        utility = avg_loss + 0.00001 * T # 0.00001  0.0000213359
...
df.to_csv(f"{path}/optimal_info_Equal_B.csv", index=False)
...
for T_val in T_vals:
    ...
    # Save plotted data to CSV
df = pd.DataFrame(save_optimal_info)
df.to_csv(f"{path}/tval_info_Equal_B.csv", index=False)
